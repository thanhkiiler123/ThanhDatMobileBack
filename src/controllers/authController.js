...
const verifyEmail = async (req, res) => {
    ...
    if (!KhachHang) {
        res.status(StatusCodes.UNAUTHORIZED).json({ msg: 'Không tìm thấy người dùng' })
        throw new CustomAPIError.UnauthenticatedError('Không tìm thấy người dùng')
    } else if (KhachHang.VerificationToken !== verificationToken) {
        res.status(StatusCodes.UNAUTHORIZED).json({ msg: 'Không đúng token' })
